    ...
    def __init__(
        ...
        **kwargs
        ...
        if kwargs_search is not None:
            checks = [["min_lon", "max_lon", "min_lat", "max_lat"],
                      ["min_time", "max_time"],]
            for check in checks:
                if any(key in kwargs_search for key in check) and not all(key in kwargs_search for key in check):
                    raise KeyError(f'If any of {check} are input, they all must be input.')
        self.kwargs_search = kwargs_search

        if category_search is not None:
            category, key = list(*category_search.items())
            # Currently just take first match, but there could be more than one.
            self.kwargs_search[category] = match_key_to_category(self.server, key, category)[0]
